...
def evaluation_benchmark(encoder, bn, decoder, dataloader,device, normal_class=0, epoch=None):
    bn.eval()
    decoder.eval()
    ...
    with torch.no_grad():
        for img, label in dataloader:
            ...
            labels = [0 if x.tolist() == normal_class else 1 for x in label]
            gt_list_sp.append(labels)
            pr_list_sp.append(np.max(anomaly_map))
        
            x_type_list.extend(label.tolist())

        auroc_sp = round(roc_auc_score(gt_list_sp, pr_list_sp), 3)
        precision, recall, thresholds = precision_recall_curve(gt_list_sp, pr_list_sp)
        prauc_sp = auc(recall, precision)

        # save
        # pred_values = {'gt_list':gt_list_sp, 'pred_list':pr_list_sp, 'img_path_list':img_path_list, 'x_type':x_type_list}
        # with open(f"checkpoints/pred_values_{_class_}_epoch{epoch}.pickle", "wb") as f:
        #     pickle.dump(pred_values, f)

    return auroc_sp, prauc_sp

def evaluation_battery(encoder, bn, decoder, dataloader,device,_class_=None, epoch=None):
    ...
    # aupro_list = []
    with torch.no_grad():
        for img, label, file_name, x_type  in dataloader:
            ...
            gt_list_sp.append(label.cpu().numpy()[0])
            pr_list_sp.append(np.max(anomaly_map))
            img_path_list.extend(file_name)
            x_type_list.extend(x_type)
...
def evaluation(encoder, bn, decoder, dataloader,device,_class_=None, epoch=None):
    #_, t_bn = resnet50(pretrained=True)
    #bn.load_state_dict(bn.state_dict())
    bn.eval()
    #bn.training = False
...
def test(_class_):
    ...
    data_transform, gt_transform = get_data_transforms(200, 200)
